...
def gumbel_softmax(logits, tau=1, hard=False, dim=-1):
    # type: (torch.Tensor, float, bool, int) -> torch.Tensor
    r"""
    This code is taken from torch.nn.functional.gumbel_softmax.
    ...
    Examples::
        ...
        >>> gumbel_softmax(logits, tau=1, hard=True)
    ...
    Example::
        ...
        if (self.training and self.generate_masking_on_train) or (not self.training and self.generate_masking_on_eval):
            ...
            if self.portions_mlm_target.device != p_mlm.device:
                self.portions_mlm_target = self.portions_mlm_target.to(p_mlm.device)
