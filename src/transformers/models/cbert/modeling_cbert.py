...
def load_tf_weights_in_cbert(model, config, tf_checkpoint_path):
    ...
    for name, array in zip(names, arrays):
        ...
        # CBERT attention was split between self and output which have been abstracted away
        ...
        name = name.replace("inner_group_", "cbert_layers/")
        name = name.replace("group_", "cbert_layer_groups/")

        # Classifier
        if len(name.split("/")) == 1 and ("output_bias" in name or "output_weights" in name):
            name = "classifier/" + name

        # No CBERT model currently handles the next sentence prediction task
...
class CBertEmbeddings(nn.Module):
    """
...
class CBertAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class CBertLayer(nn.Module):
    def __init__(self, config):
        ...
        self.attention = CBertAttention(config)
...
class CBertLayerGroup(nn.Module):
    def __init__(self, config):
        super().__init__()

        self.cbert_layers = nn.ModuleList([CBertLayer(config) for _ in range(config.inner_group_num)])
    ...
    def forward(
        ...
        for layer_index, cbert_layer in enumerate(self.cbert_layers):
            layer_output = cbert_layer(hidden_states, attention_mask, head_mask[layer_index], output_attentions)
...
class CBertTransformer(nn.Module):
    def __init__(self, config):
        super().__init__()

        self.config = config
        self.embedding_hidden_mapping_in = nn.Linear(config.embedding_size, config.hidden_size)
        self.cbert_layer_groups = nn.ModuleList([CBertLayerGroup(config) for _ in range(config.num_hidden_groups)])
    ...
    def forward(
        ...
        for i in range(self.config.num_hidden_layers):
            ...
            layer_group_output = self.cbert_layer_groups[group_idx](
                hidden_states,
                attention_mask,
...
class CBertModel(CBertPreTrainedModel):
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.config = config
        self.embeddings = CBertEmbeddings(config)
        self.encoder = CBertTransformer(config)
